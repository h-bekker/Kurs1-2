...
char* getword(int* kav, int* amp, char* ch, int* trans) {
	int n=1, pos=0;
	char c='\0';
	char* str=NULL;
...
char* getlines(int* flag, int* amp, int* trans) {
	char* str=NULL;
	char c='\0';
	int kav=0;
	str=getword(&kav, amp, &c, trans);
	if (c=='\n')
		*flag=1;
	if (kav==1) {
...
void mkexec(char** argv, int k, int pid1, int trans, struct node* fortr) {
...
void cmd(struct node* l, int trans,struct node* fortr) {
	int pid=0,i=0,n=1;
	char** arg=NULL;
	struct node* link=l;
	if (l==NULL)
...
int main(int argc, char** argv) {
	...
	struct node* fortr = NULL;
	char* str;
	int flag, amp=0, trans=0;
	while (!feof(stdin)) {
		flag=0;
		str=getlines(&flag, &amp, &trans);
		if (str!=NULL) {
			link=newl(str);
			if (trans)
				fortr=link;
